...
!function(t){function e(t){var e=(t.match(/<\s*([^>\s]+)[\s\S]*?>/)||[0,!1]).splice(1),i=e.some(function(t){return t&&CKEDITOR.dtd.$block[t]}),n="span";return i&&(n="div"),n}function i(e,i){e.each(function(e,n){var s,l=t(n);s=t("<"+i+">"),t.each(n.attributes,function(t,e){s.attr(e.nodeName,e.nodeValue)}),s.html(l.html()),l.replaceWith(s)})}CKEDITOR&&CKEDITOR.plugins&&CKEDITOR.plugins.registered&&CKEDITOR.plugins.registered.cmsplugins||CKEDITOR.plugins.add("cmsplugins",{icons:"cmsplugins",init:function(e){var i=this;if(this.options=CMS.CKEditor.options.settings,this.editor=e,this.child_plugins=[],this.setupCancelCleanupCallback(this.options),void 0===this.options||void 0===this.options.plugins)return!1;this.setupDialog(),this.editor.ui.add("cmsplugins",CKEDITOR.UI_PANELBUTTON,{toolbar:"cms,0",label:this.options.lang.toolbar,title:this.options.lang.toolbar,className:"cke_panelbutton__cmsplugins",modes:{wysiwyg:1},editorFocus:0,panel:{css:[CKEDITOR.skin.getPath("editor")].concat(i.editor.config.contentsCss),attributes:{role:"cmsplugins","aria-label":this.options.lang.aria}},onBlock:function(e,n){n.element.setHtml(i.editor.plugins.cmsplugins.setupDropdown()),t(n.element.$).find(".cke_panel_listItem a").bind("click",function(n){n.preventDefault(),i.addPlugin(t(this),e)})}}),this.editor.contextMenu&&this.setupContextMenu(),this.editor.addCommand("cmspluginsEdit",{exec:function(){var t=i.getElementFromSelection(),e=i.getPluginWidget(t);console.log("cmspluginsEdit",t,e),e&&i.editPlugin(e)}});var n=function(e){var n;if("touchend"===e.type||"click"===e.type){var s=t(e.currentTarget).closest("cms-plugin")[0];console.log("plugin edit",s,s.parentElement),n=new CKEDITOR.dom.element(s).getParent(),e.data=e.data||{},i.editor.getSelection().fake(n)}i.editor.execCommand("cmspluginsEdit")};this.editor.on("doubleclick",n),this.editor.on("instanceReady",function(){}),this.setupDataProcessor()},getElementFromSelection:function(){var t=this.editor.getSelection();return t.getSelectedElement()||t.getCommonAncestor().getAscendant("cms-plugin",!0)},getPluginWidget:function(t){return t?t.getAscendant("cms-plugin",!0)||t.findOne("cms-plugin"):null},setupDialog:function(){var e=this,i=function(){return{title:"",minWidth:200,minHeight:200,contents:[{elements:[{type:"html",html:'<iframe style="position:static; width:100%; height:100%; border:none;" />'}]}],onOk:function(){var i=t(CKEDITOR.dialog.getCurrent().parts.contents.$).find("iframe").contents(),n=i[0].URL;i.find("form").submit();var s=CMS.API.Helpers.onPluginSave;return CMS.API.Helpers.onPluginSave=function(){CKEDITOR.dialog.getCurrent().hide();var t=CMS.API.Helpers.dataBridge,i=!1;return n.match(/add-plugin/)&&(i=!0),!e.options.delete_on_cancel&&i&&e.child_plugins.push(t.plugin_id),e.insertPlugin(t),CMS.API.Helpers.onPluginSave=s,!1},!1}}};CKEDITOR.dialog.add("cmspluginsDialog",i)},setupDropdown:function(){var e='<div class="cke_panel_block">';return t.each(this.options.plugins,function(i,n){e+='<h1 class="cke_panel_grouptitle">'+n.group+"</h1>",e+='<ul role="presentation" class="cke_panel_list">',t.each(n.items,function(t,i){e+='<li class="cke_panel_listItem"><a href="#" rel="'+i.type+'">'+i.title+"</a></li>"}),e+="</ul>"}),e+="</div>"},setupContextMenu:function(){var t=this;this.editor.addMenuGroup("cmspluginsGroup"),this.editor.addMenuItem("cmspluginsItem",{label:this.options.lang.edit,icon:CMS.CKEditor.options.settings.static_url+"/ckeditor_plugins/cmsplugins/icons/cmsplugins.svg",command:"cmspluginsEdit",group:"cmspluginsGroup"}),this.editor.removeMenuItem("image"),this.editor.contextMenu.addListener(function(e){if(t.getPluginWidget(e))return{cmspluginsItem:CKEDITOR.TRISTATE_OFF}})},editPlugin:function(e){var i=e.getAttribute("id");this.editor.openDialog("cmspluginsDialog");var n=CMS.$(window),s=CKEDITOR.dialog.getCurrent();s.resize(.8*n.width(),.7*n.height()),t(s.getElement().$).addClass("cms-ckeditor-dialog"),t(s.parts.title.$).text(this.options.lang.edit),console.log("id",i),console.log("this",this);var l=this.options.url||window.location.href,o=encodeURIComponent(window.parent.location.pathname+window.parent.location.search),a=l.replace(/(add-plugin|edit-plugin).*$/,"edit-plugin/"+i+"/?_popup=1&no_preview&cms_history=0&cms_path="+o);console.log("path",o,"textPluginUrl",l,"childPluginUrl",a),console.log(this.editor),t(s.parts.contents.$).find("iframe").attr("src",a).bind("load",function(){var e=t(this).contents();e.find("body").addClass("ckeditor-popup"),e.find(".submit-row").hide(),e.find("#container").css({"min-width":0,padding:0})})},addPlugin:function(t,e){var i=this;console.log("this",this),e.hide(),this.editor.focus(),this.editor.fire("saveSnapshot");var n={placeholder_id:this.options.placeholder_id,plugin_type:t.attr("rel"),plugin_parent:this.options.plugin_id,plugin_language:this.options.plugin_language,cms_path:window.parent.location.pathname,cms_history:0};i.addPluginDialog(t,n)},addPluginDialog:function(e,i){var n=t(window),s=this.editor.getSelection().getSelectedText();this.editor.openDialog("cmspluginsDialog");var l=CKEDITOR.dialog.getCurrent();l.resize(.8*n.width(),.7*n.height()),t(l.getElement().$).addClass("cms-ckeditor-dialog"),t(l.parts.title.$).text(this.options.lang.add),t(l.parts.contents.$).find("iframe").attr("src",this.options.add_plugin_url+"?"+t.param(i)).on("load.addplugin",function(){var e=t(this),i=e.contents();i.find(".submit-row").hide().end().find("#container").css("min-width",0).css("padding",0);var n=i.find(".js-ckeditor-use-selected-text");n.length||(n=i.find("#id_name")),n.val()&&n.val().trim()||n.val(s),e.off("load.addplugin")})},insertPlugin:function(e){var i=this;t.ajax({method:"GET",url:i.options.render_plugin_url,data:{token:i.options.action_token,plugin:e.plugin_id}}).done(function(t){i.editor.insertHtml(t,"unfiltered_html"),i.editor.fire("updateSnapshot")})},setupCancelCleanupCallback:function(){if(window.parent&&window.parent.CMS&&window.parent.CMS.API&&window.parent.CMS.API.Helpers){var e=this,i=window.parent.CMS,n=function(n,s){if((e.options.delete_on_cancel||e.child_plugins.length)&&e.child_plugins.length){n.preventDefault(),i.API.Toolbar.showLoader();var l={token:e.options.action_token};e.options.delete_on_cancel||(l.child_plugins=e.child_plugins),t.ajax({method:"POST",url:e.options.cancel_plugin_url,data:l,traditional:!0}).done(function(){i.API.Helpers.removeEventListener("modal-close.text-plugin.text-plugin-"+e.options.plugin_id),s.instance.close()}).fail(function(t){i.API.Messages.open({message:t.responseText+" | "+t.status+" "+t.statusText,delay:0,error:!0})})}};i.API.Helpers.addEventListener("modal-close.text-plugin.text-plugin-"+e.options.plugin_id,n)}},setupDataProcessor:function(){function n(t){return t.attributes&&"div"===t.attributes["data-cke-real-element-type"]}var s=this;CKEDITOR.htmlParser.element=CKEDITOR.tools.override(CKEDITOR.htmlParser.element,function(t){return function(e,i){t.call(this,e,i),"cms-plugin"===e&&"span"===i["data-cke-real-element-type"]&&(this._.isBlockLike=!1)}}),this.editor.dataProcessor.dataFilter.addRules({elements:{span:function(e){if(CKEDITOR.plugins.widget.isParserWidgetWrapper(e)){var i=e.getFirst();if(n(i)){var l=new CKEDITOR.htmlParser.element("div",t.extend({},e.attributes));return s.editor.widgets.registered.cmswidget.inline=!1,l.children=e.children,l.removeClass("cke_widget_inline"),l.removeClass("cke_widget_force_block"),l.addClass("cke_widget_block"),i.attributes["data-cke-real-element-type"]="div",l}s.editor.widgets.registered.cmswidget.inline=!0,i.attributes["data-cke-real-element-type"]="span"}return e}}},{priority:1,applyToAll:!0}),this.editor.on("toHtml",function(n){var s=n.data.dataValue.replace(/<cms-plugin(.*?)>([\s\S]*?)<\/cms-plugin>/gi,function(t,i,n){var s=e(n);return i.match(/render-plugin=["']?false/gi)?'<cms-plugin data-cke-real-element-type="'+s+'" '+i+"><"+s+' class="cms-ckeditor-plugin-label">'+i.replace(/[\s\S]*alt=["']([\s\S]*?)['"][\s\S]*/,"$1")+"</"+s+"></cms-plugin>":'<cms-plugin data-cke-real-element-type="'+s+'" '+i+">"+n+"</cms-plugin>"});if(s.match(/<cms-plugin[^>]*(?=data-cke-real-element-type=\"div)/)){var l=/<cms-plugin([^>]*(?=data-cke-real-element-type=\"div)[\s\S]*?>[\s\S]*?<\/)cms-plugin>/g,o=s.replace(l,"<div$1div>"),a=t(o).wrapAll("<div>").parent();i(a.find('div[data-cke-real-element-type="div"]'),"cms-plugin"),s=a.html()}n.data.dataValue=s},null,null,4),this.editor.on("toHtml",function(){s.editor.widgets&&s.editor.widgets.registered&&s.editor.widgets.registered.cmswidget&&(s.editor.widgets.registered.cmswidget.inline=!0)},null,null,1)}})}(CMS.$);
});
})(CMS.$);
